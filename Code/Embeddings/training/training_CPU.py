...
def train(train_loader, model, criterion, optimizer, epoch, ntokens):
    ...
    for i, (input_data, target) in enumerate(train_loader):
        ...
        input_data = Variable(torch.squeeze(input_data))
        target = Variable(torch.squeeze(target))
...
class LSTMLanguageModeler(nn.Module):
    ...
    def init_embeddings(self, pre_trained_weights_np):
        ...
        pre_trained_weights_tensor = torch.from_numpy(pre_trained_weights_np)
        ...
        dummy_weights_tensor = torch.FloatTensor(self.new_words_len, self.embedding_dim)
    ...
    def init_hidden(self, batch_size):
        ...
        return (Variable(torch.zeros(1, batch_size, self.hidden_dim), requires_grad=False),
                Variable(torch.zeros(1, batch_size, self.hidden_dim), requires_grad=False))
    ...
    def get_sequence_static_embeddings(self, sequence):
        ...
        for num in sequence.data:  # .data to access tensor inside Variable obj
            if num < self.pre_trained_vocab_len:
                embeddings_sequence.append(self.static_embedding(Variable(torch.LongTensor([num]))))
            else:
                embeddings_sequence.append(self.static_dummy_embedding(Variable(
                    torch.LongTensor([num - self.pre_trained_vocab_len]))))
