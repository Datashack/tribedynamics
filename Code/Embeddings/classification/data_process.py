...
def get_dataframe_by_brand_id(brand_id):
    ...
    with codecs.open(PATH_TO_LABELED_DATA + str(brand_id) + '_data.json', 'r', 'utf-8') as f_data:
        dict_list = json.load(f_data, encoding='utf-8')

    return pd.DataFrame.from_dict(dict_list)


def get_list_of_brand_id_with_model_decision():
    brand_ids = []

    # Loop over all the file ids provided
    for brand_id in range(FIRST_BRAND_ID, LAST_BRAND_ID + 1):
        # Check existence of file
        try_file = Path(PATH_TO_LABELED_DATA + str(brand_id) + "_data.json")

        if try_file.is_file():
            # Get dataset from file
            df_full = get_dataframe_by_brand_id(brand_id)
            if len(Counter(df_full.model_decision.values)) == 2:
                brand_ids.append(brand_id)

    return brand_ids


def get_list_of_brand_id():
    brand_ids = []
...
def output_performance_metrics(y_true, y_pred):
    ...
    print("- ROC curve AUC = {:.2f}".format(roc_auc_score(y_true, y_pred)))
    print("- Average precision score = {:.2f}".format(average_precision_score(y_true, y_pred)))
...
def get_tribe_performance(df, languages_list):
    df = filter_df_by_languages(df, languages_list)
    y_true = df.answer.values
    y_pred = df.model_decision.values
...
def clean_corpus(corpus_arr, encode_social_chars):
