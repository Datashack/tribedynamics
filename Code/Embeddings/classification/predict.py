...
def cv_evaluation_avg(X, y, n_splits, model_obj, random_state=RANDOM_STATE):
    ...
    for train_ix, test_ix in folds.split(X, y):
        clf = deepcopy(model_obj)  # TODO might be unnecessary to deepcopy object
        trained_model = clf.train(X[train_ix], y[train_ix])
        results_dict = compute_eval_metrics(X[test_ix], y[test_ix], trained_model)
    ...
    return {'accuracy': np.mean(acc_scores),
...
def averaged_cv_perf_list(perf_list, n_splits):
    splitted_list = split_list(perf_list, int(len(perf_list)/n_splits))
    return [np.mean(sub_list) for sub_list in splitted_list]
